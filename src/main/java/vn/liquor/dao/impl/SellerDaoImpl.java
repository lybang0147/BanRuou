...
public class SellerDaoImpl implements SellerDao {
	...
	public int daban(int sid) {
		String sql="select sum(sellamount) from seller inner join Product on Product.sellerId=seller.id \r\n" + 
				"where seller.id=?";
		try
		{
			conn=new DBConnect().getConnection();
	...
	public int tongdoanhthu(int sid) {
		String sql="select sum(cartdetail.subtotal) from seller inner join Product on Product.sellerId=seller.id inner join Cartdetail on Product.productId=Cartdetail.productId\r\n" + 
				"where seller.id=? and Cartdetail.status=1";
		try
		{
			conn=new DBConnect().getConnection();
			ps=conn.prepareStatement(sql);
	...
	public int tongsanpham(int sid) {
		String sql="select count(productid) from seller inner join Product on Product.sellerId=seller.id \r\n" + 
				"where seller.id=?";
		try
		{
			conn=new DBConnect().getConnection();
