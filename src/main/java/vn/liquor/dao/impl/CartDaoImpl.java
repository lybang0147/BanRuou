...
public class CartDaoImpl extends DBConnect implements CartDao {
	...
	public CartModel get(String id) {
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next())
				return new CartModel(rs.getString(1), userservice.get(rs.getInt(2)), rs.getDate(3), rs.getInt(4));
		} catch (Exception e) {
	...
	public List<CartModel> getcartbyuid(int uid) {
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next())
				list.add(new CartModel(rs.getString(1), userservice.get(rs.getInt(2)), rs.getDate(3), rs.getInt(4)));
		} catch (Exception e) {
			// TODO: handle exception
		}
		return list;
