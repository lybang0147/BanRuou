...
public class CartItemDaoImpl implements CartItemDao {
	...
	public CartItemModel get(int id) {
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next())
				return new CartItemModel(rs.getInt(1), cartservice.get(rs.getString(2)),
						productservice.getproductbypid(rs.getInt(3)), rs.getInt(4), rs.getInt(5), rs.getInt(6),
						rs.getInt(7));
		} catch (Exception e) {
			// TODO: handle exception
		}
	...
	public List<CartItemModel> getAllCartItem(String id) {
		...
		try {
			...
			while (rs.next()) {
				CartModel cart = cartservice.getCart(rs.getString(2));
				ProductModel product = productservice.getproductbypid(rs.getInt(3));
